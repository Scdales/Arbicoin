...
class Public(BaseClient):
    ...
    def book(self, coin):
        ...
        """
        if coin == 'ltcbtc':
            return self._get("v1/book/ltcbtc", return_json=True)
        if coin == 'ethbtc':
            return self._get("v1/book/ethbtc", return_json=True)
        if coin == 'etcbtc':
            return self._get("v1/book/etcbtc", return_json=True)
...
class Trading(Public):
    ...
    #Key: 8Y9iGOlit2RTTdLoVdyxkuLLGk15liWVb1eB8O0gu3Y
    #Secret: z2r9ESyW9W47IrsaPuLZzHmLknfLAqpDIoYXsnPgwLd
    ...
    def new_order(self, amount, price, side,
                  order_type, symbol):
        """
    ...
    def withdraw(self, cointype, address, amount, exchange):
        """
        withdraw_type 	[string] 	can be "bitcoin", "litecoin"
        or "ethereum" or "tether" or "wire".
        walletselected 	[string] 	The wallet to withdraw from,
        can be "trading", "exchange", or "deposit".
        amount 	[string] 	Amount to withdraw.
        address [string] destination address
        """
        #exchange = 'exchange'    #done in higher script
        
        data = {'withdraw_type': str(cointype),
                'amount': str(amount),
                'walletselected': str(exchange),
                'address': str(address)
                }
